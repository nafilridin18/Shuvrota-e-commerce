...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_profile'])) {
    ...
    elseif (!empty($email) && !filter_var($email, FILTER_VALIDATE_EMAIL)) {
        $errorMessage = "সঠিক ইমেইল এড্রেস দিন। / Please enter a valid email address.";
    }
    else {
        try {
            ...
            if (!empty($email)) {
                ...
                $dup->execute([$email, $customerId]);
                if ($dup->fetch()) {
                    $errorMessage = "এই ইমেইলটি দিয়ে ইতিমধ্যে আরেকটি অ্যাকাউন্ট রয়েছে। / This email is already registered to another account.";
                }
            }

            if (empty($errorMessage)) {
                $stmt = $pdo->prepare("UPDATE customers SET name = ?, phone = ?, email = ?, address = ?, city = ?, postal_code = ? WHERE id = ?");
                $stmt->execute([
                    $name,
                    $phone,
                    $email !== '' ? $email : null,
                        ...
                        <form action="account.php" method="POST">
                            ...
                            <div class="row g-3">
                                ...
                                <div class="col-md-12">
                                    ...
                                    <div class="account-email-hint">
                                        <i class="fa-solid fa-circle-info"></i>
                                        <span>
                                            <span class="lang-bn">ঐচ্ছিক — অর্ডার আপডেট এবং যোগাযোগের জন্য ব্যবহৃত হবে।</span>
                                            <span class="lang-en">Optional — used for order updates and communication.</span>
                                        </span>
                                    </div>
                                </div>
                                <div class="col-md-12">
                                                  ...
                                                  <?= csrf_field() ?>
                                                     ...
                                                     <div class="modal-body">
                                                        ...
                                                        <div class="mb-3">
                                                            ...
                                                            <input type="text" name="subject" class="form-control" required placeholder="যেমন: সাইজ বা ডেলিভারি নিয়ে মন্তব্য">
                                                        </div>
                                                        <div class="mb-3">
